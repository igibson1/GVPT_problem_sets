...
::: {.callout-tip title="HG comments"}
Great script management: good to put them up front. Just remember to only load in the packages you will use this session. 
:::
...
::: {.callout-tip title="HG comments"}
Nice graph! Be careful with your labelling. `libpct_m` is provided in percentage points, not frequency. 
:::
...
::: {.callout-tip title="HG comments"}
Correct! But there is some information that you can provide here, including the averages and standard deviations of `libpct_m` and `womleg_2015`.
:::
...
::: {.callout-tip title="HG comments"}
Sounds good, but I would like more description of the scatter plot itself. Do you see a positive association? Are the points tightly clustered around each other or more spread out? 
:::
...
::: {.callout-tip title="HG comments"}
Almost! You get it right below, but above you are mixing up the dependent variable, `womleg_15` and the intercept coefficient, $\beta_0$, and mixing up the dependent variable, `libpct_m` and its coefficient, $\beta_1$. 
...
Which you correctly identify here:
:::
...
::: {.callout-tip title="HG comments"}
A professionally formatted table will include variable labels that are easy to interpret. I recommend redoing `Public_liberal`. Otherwise, good table. 
:::
...
::: {.callout-tip title="HG comments"}
We were looking for the observed values of the `womleg_2015` (dependent) variable, which you need to calculate the residual, but the question isn't clear so I haven't taken away any points. 
:::
...
::: {.callout-tip title="HG comments"}
The question also asks for the total aggregate error from those two select observations combined.
...
::: {.callout-tip title="HG comments"}
It is often better to describe the relationship in proportional terms instead of counts. This makes comparing across and within groups easier. 
:::
...
::: {.callout-tip title="HG comments"}
Make sure your ordered categorical variables are provided in a meaningful order: low, mid, then high. 
:::
